...
function xlsx2sql(file, config, callback) {
    ...
    config.fields.forEach(function(field) {
        if (typeof field.value === 'undefined' && typeof field.index === 'undefined' || !field.name) throw new Error('Bad config, please specify a "name" and ("index" or "value") for each ligne')
    });
        ...
        page.forEach(function(line) {
            ...
            config.fields.forEach(function(field) {
                if (typeof field.value !== 'undefined') x.push(field.value);
                    ...
                    if (field.convert)
                        x.push(escaping(field.convert(line[field.index])));
                    else
                        x.push(escaping(line[field.index]));
                }
            });
            values.push(x)
        })
