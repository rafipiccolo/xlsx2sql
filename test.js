    ...
    it('should generate sql', function(cb) {
        ...
        xlsx2sql.xlsx2sql(`${__dirname}/test.xlsx`, {
            ...
            fields: [
                ...
                { index: 2, name: 'tva_id', convert: function(v) {
                    if (v == '0.1') return 60;
                ...
                { index: 4, name: 'tarifLibreBoolean', convert: function(v) {
                    if (v == 'ON') return 1;
                    else return 0;
                }},
                { index: 5, name: 'compta' },
            ...
            assert.ifError(err);
            assert.equal(sql, "insert into `product` (`title`,`start`,`tva_id`,`tarifLibreTtc`,`tarifLibreBoolean`,`compta`,`createdAt`,`updatedAt`) values\n"+
